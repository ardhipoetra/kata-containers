...
#[allow(dead_code)]
async fn do_kernel_dice_init_key(logger: &Logger) -> bool {
    ...
    unsafe {
        ...
        warn!(logger,"RDKATA > ioctl SCONE_IOC_DICE_INIT_NEW return : {:?}", ret);
    }
...
async fn do_kernel_stuff(logger: &Logger) -> bool {
    ...
    let mut args = scbindings::scone_dice_init {
    ...
    nix::ioctl_readwrite!(sc_dice_init, b'a', 12, scbindings::scone_dice_init);
    ...
    unsafe {
        let ret = sc_dice_init(f.as_raw_fd(), &mut args);
        warn!(logger,"RDKATA > ioctl SCONE_IOC_DICE_INIT return : {:?} signature: {:02X?} measurement:{:02X?}", ret, 
    ...
    //     warn!(logger,"RDKATA > ioctl SCONE_IOC_DICE_INIT_NEW return : {:?}", ret);
    // }
    // end SCONE_IOC_DICE_INIT_NEW


    // do SCONE_IOC_DICE_INIT_GET
    // let mut args_init_get = scbindings::scone_dice_init_get {
    //     pubkey: [0u8;32],
    // }; 

    // nix::ioctl_read!(sc_init_get, b'a', 17, scbindings::scone_dice_init_get);
    // let f = {
    //     let fd = nix::fcntl::open("/dev/scone_enclave", OFlag::O_RDONLY, nix::sys::stat::Mode::all());
    //     // Wrap fd with `File` to properly close descriptor on exit
    //     unsafe { fs::File::from_raw_fd(fd.expect("fd errr")) }
    // };
    // unsafe {
    //     let ret = sc_init_get(f.as_raw_fd(), &mut args_init_get);
    //     warn!(logger,"RDKATA > ioctl SCONE_IOC_DICE_INIT_GET return : {:?}", ret);
    //     warn!(logger,"RDKATA > pubkey from INIT_GET: {:02X?}", 
    //         args_init_get.pubkey,
    //     );
    // }
    // end SCONE_IOC_DICE_INIT_GET
